...
2026-09-14 本地只读结构检查发现，`sibebid.bi_order_issue_year` 当前有 41 个字段，但尚未包含 `issue_profit`。因此 MySQL 模式下出票利润、出票负利润问题和总预估利润暂不可用，等待同步该权威利润字段；不能擅自用收入减成本推导替代。当前 MySQL 版本也不支持窗口函数，问题中心已采用兼容的“汇总查询 + Top 明细查询”。
