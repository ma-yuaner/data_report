...
| 退票 | `lywz.dwd_refund_issue_year` | `apply_datetime` | `count(1)` | `sum(refund_profit)` | `business_type_desc in ('正常退票（退票）','售后退票作废（退票）')`；`supplier_refund_operator is not null` 且去除首尾空格后不为空 |
